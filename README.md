...
供家人共同規劃、旅途中隨手查閱的福岡五天四夜旅行 PWA。整合每日行程、釘選路線地圖、共同清單、伴手禮、折價券、發票辨識帳本與私人文件捷徑，兼顧電腦規劃與手機現場使用。

**旅行日期：2026/09/09–2026/09/13** · **介面：繁體中文** · **目前 Service Worker 版本：5.9.0**
...
| 功能 | 說明 |
| --- | --- |
| 首頁與下一站 | 出發倒數、當日行程提示、福岡即時天氣、七日預報與常用入口；下一站會使用同步後的行程資料 |
...
| 共用折價券夾 | 貼上、拖曳或從相簿選圖，依店家收折，支援期限、網址、備註與放大掃碼 |
| 發票掃描與旅費 | 用相機或相簿辨識日本發票，保留日文原文、繁體中文翻譯、品項與日圓金額；確認後同步給家人帳號 |
...
所有訪客均可從旅遊資訊中心開啟折價券夾，免登入瀏覽與新增；網站使用既有 Firebase 設定自動同步。編輯與刪除仍限家人帳號。選圖後會先在裝置縮圖，補上名稱、店家、期限、優惠網址與提醒，再儲存至公開共用折價券夾。

- 檔案選擇器支援 PNG、JPEG、WebP；原始圖片上限 15 MB。
- 前端新增上限為 24 張（不是伺服器總量限制）；縮圖後的圖片 Data URL 目標上限為 550,000 字元。
- 圖片存於 Firestore `coupons` 文件，不使用 Firebase Storage。
- 點圖片可放大供店員掃碼；期限標記只依填寫日期判斷，不會向店家驗證優惠。

### 發票掃描與家人共用帳本

發票工具位於首頁「快速入口」與「旅遊資訊中心」。登入家人帳號後，可直接拍攝或從相簿選擇日本發票；網站會在目前裝置使用 Tesseract.js 日文 OCR，解析店家、日期、品項、稅額與總額，再以內建常見品項字典提供繁體中文翻譯。所有結果都會先進入確認表單，可修正、增刪品項後再儲存。

- 發票照片只暫存在目前頁面，不寫入 Firestore、Firebase Storage 或 localStorage。
- Firestore 只保存確認後的文字、品項、數量、日圓金額、備註與修改者；依需求不另做欄位加密。
- 第一次 OCR 需連網載入 Tesseract.js 與日文辨識模型；模型可由瀏覽器快取，但不同裝置仍需各自下載。
- 自動翻譯採旅行購物常見詞彙對照。未命中的品項會保留日文並顯示確認提示，不會假裝已完整翻譯。
- 發票帳本只允許 Firestore 安全規則驗證過的家人帳號讀寫。
...
| `receipt-scanner.js`、`receipt-scanner.css` | 裝置端日文 OCR、品項解析／翻譯、發票確認表單與共用帳本介面 |
...
| `coupons` | 目前公開旅程可讀取及新增；新增資料需通過欄位驗證 | 讀寫 |
| `receipts` | 不可讀寫 | 讀寫；只保存確認後的文字與金額，不含發票圖片 |
...
更新應用程式或本機資源時，檢查 `sw.js` 的 `VERSION`、`APP_SHELL` 與 HTML／manifest 的資源路徑。現行版本為 `5.9.0`，manifest 與發票工具查詢版本為 `v=590`；字型另使用 `v=523`。新增或刪除預快取資源時，需同步調整清單，任何必要檔案 404 都可能導致安裝失敗。

更新後若仍看到舊介面，可重新整理或關閉後重開 PWA。清除網站資料會影響本機偏好與離線資料，操作前先確認資料已同步。

常用直達入口：`?page=trip&day=1`、`?page=checklist`、`?page=gifts`、`?page=more`、`?page=emergency`、`?page=flight`、`?page=hotel`、`?page=exchange`、`?page=secure`、`?page=family`、`?page=coupons`、`?page=receipts`。
...
| 發票 OCR | 第一次需下載程式與日文模型；照片不寫入快取或雲端，確認後的帳本資料依 Firestore 離線能力處理 |
...
- 發票相機／相簿選擇、OCR 失敗回退、品項修正、金額不一致提示與家人同步。
- PWA 安裝、更新、離線重新開啟與外部服務失敗回退。

本 README 依儲存庫 v5.9.0 程式與文件整理，不代表已驗證線上 Firebase 設定或所有裝置功能。
