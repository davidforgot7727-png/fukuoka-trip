...
**旅行日期：2026/09/09–2026/09/13** · **介面：繁體中文** · **目前 Service Worker 版本：5.8.4**
...
更新應用程式或本機資源時，檢查 `sw.js` 的 `VERSION`、`APP_SHELL` 與 HTML／manifest 的資源路徑。現行版本為 `5.8.4`，manifest 查詢版本為 `v=584`；字型另使用 `v=523`。新增或刪除預快取資源時，需同步調整清單，任何必要檔案 404 都可能導致安裝失敗。
...
本 README 依儲存庫 v5.8.4 程式與文件整理，不代表已驗證線上 Firebase 設定或所有裝置功能。
